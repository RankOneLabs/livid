...
In a cycle, declaration order is reading order: the loop reads from its
first-listed node, and the edge back to an earlier-listed node is the one that
wraps. Acyclic specs are ranked by their edges alone.
